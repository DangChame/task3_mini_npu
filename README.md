...
측정에 쓰는 패턴과 필터는 **크기마다 생성기로 만듭니다.** MAC은 값이 무엇이든 곱셈
횟수가 같아서(`0.0 × 0.0`이든 `7.3 × 2.1`이든 한 번은 한 번) 걸리는 시간이 **데이터
내용이 아니라 크기 N에만 좌우되기 때문**입니다. O(N²)를 보이려면 여러 크기가 필요한데
`data.json`에는 5·13·25만 있고 3×3이 없어서 생성기로 채웠습니다.
