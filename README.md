...
과제 요구사항 「4. 기능 요구 사항 → 1. 데이터 구조」에 이렇게 되어 있습니다.
...
`get`이 "읽어올", `set`이 "저장하고"에 해당합니다. 둘 다 실제로 쓰입니다.

- **`get`** — `mac()`이 곱셈할 때마다 부릅니다.
- **`set`** — 패턴 생성기 `make_cross`, `make_x`가 부릅니다.

생성기는 **0으로 깔아놓고 필요한 칸만 1로 바꾸는** 방식입니다.
...
모든 칸을 돌면서 "여기가 십자가인가?"를 판단하는 대신, **1이 들어갈 자리를 직접
지정**합니다. 십자가가 어디인지는 이미 알고 있으니 판단할 필요가 없습니다.
반복문이 2중에서 1중으로 줄고, 무엇을 만들려는지가 코드에 그대로 드러납니다.

`make_empty`에는 주의할 점이 하나 있습니다.

```python
rows.append([0.0] * size)     # 행마다 새 리스트를 만든다
```

짧게 쓰려고 `[[0.0] * size] * size`라고 하면 안 됩니다. 바깥쪽 `* size`는 리스트를
복사하는 게 아니라 **같은 리스트를 가리키는 참조를 늘립니다.** 행이 3개가 아니라 한
행을 세 번 가리키는 상태가 되어서, `set(0, 0, 1.0)` 한 번에 세 행이 전부 바뀝니다.

### 일부러 만들지 않은 메서드 — `FlatMatrix`의 접근자
...
| | `Matrix`의 `get`/`set` | `FlatMatrix`의 접근자 |
|---|---|---|
| 요구사항인가 | **그렇다** | 아니다 (보너스) |
| 쓰이나 | `mac()`과 패턴 생성기가 쓴다 | 만들지 않았다 |
| 왜 이렇게 했나 | 요구사항이고, **거쳐서 쓰는 게 자연스럽다** | 거치면 **최적화가 사라진다** |
...
측정에 쓰는 데이터는 모드마다 다릅니다. **모드 1은 입력받은 패턴과 필터 A를 그대로**
쓰고, 모드 2는 크기마다 생성기로 만들어 씁니다. MAC은 값이 무엇이든 곱셈 횟수가
같아서(`0.0 × 0.0`이든 `7.3 × 2.1`이든 한 번은 한 번) **걸리는 시간이 데이터 내용이
아니라 크기 N에만 좌우되기 때문에**, 어느 쪽이든 같은 크기면 결과가 같습니다.

모드 2에서 생성기를 쓰는 이유는 O(N²)를 보이려면 **여러 크기가 필요한데** 5·13·25만
`data.json`에 있고 3×3이 없어서입니다.

`print_performance`는 패턴과 필터를 **짝으로 묶어서** 받습니다.

```python
def print_performance(sizes, data=None):    # data는 (패턴, 필터)
    ...
    target, other = data
```

둘을 따로 받으면 **한쪽만 넘기는 상태**가 만들어집니다. 그러면 넘긴 값이 조용히
무시되거나 `None`을 계산하려다 죽습니다. 하나로 묶으면 그런 호출 자체가 불가능해지고,
개수가 안 맞을 때 파이썬이 `TypeError`로 바로 잡아줍니다. 검사 코드를 따로 쓰지 않아도
됩니다.
...
`make_empty(size)`로 0을 깔아놓고 `set()`으로 1이 들어갈 자리만 지정합니다.
십자가는 가운데 행과 열, X는 두 대각선(`(i, i)`와 `(i, size-1-i)`)입니다.
...
- **성능 분석 (모드 2)** — 크기마다 십자가 패턴과 X 필터를 만들어 MAC 시간을 잽니다.
  모드 1은 생성기 대신 **입력받은 패턴과 필터 A**로 잽니다.
