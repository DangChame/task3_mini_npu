...
`Matrix` 클래스가 n×n 배열을 담습니다. 2차원 리스트를 들고 있고 `get(row, col)`으로
값을 꺼냅니다. `is_square()`로 행마다 길이가 `size`와 같은지 검사해서, 들쭉날쭉한
배열이 MAC 연산에 들어가 `IndexError`로 죽는 일을 막습니다. `show()`는 입력받은
행렬을 확인용으로 출력합니다.
...
`FlatMatrix`에는 `get`/`set` 같은 접근자를 두지 않았습니다. 만들어두면 "쓰라고 있는
것"으로 보이는데 정작 쓰면 안 되는 메서드가 되어서, 아예 없는 편이 의도가 분명합니다.
